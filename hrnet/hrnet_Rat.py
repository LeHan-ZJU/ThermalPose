...
from hrnet.modules import BasicBlock, Bottleneck
        ...
        for i in range(len(self.fuse_layers)):
            for j in range(0, len(self.branches)):
                if j == 0:
                    ...
                    # print(x_fused[i].shape, self.fuse_layers[i][j](x[j]).shape)
...
class HRNet(nn.Module):
    def __init__(self, c, n_channels, nof_joints, bn_momentum):
        ...
        self.final_layer = nn.Conv2d(c, self.n_classes, kernel_size=(1, 1), stride=(1, 1))
        nn.init.normal_(self.final_layer.weight, std=0.001)
        nn.init.constant_(self.final_layer.bias, 0)
        # for m in self.predict.modules():  # [ZL] important
        #      if isinstance(m, nn.Conv2d):                # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')  # NOT
...
class HRNet_loc(nn.Module):
    def __init__(self, c, n_channels, nof_joints, bn_momentum):
        super(HRNet_loc, self).__init__()

        self.n_channels = n_channels
        self.n_classes = nof_joints

        # 初始化定位输出层参数
        self.outConvLoc = nn.Conv2d(32, 1, kernel_size=(1, 1), stride=(1, 1))
        nn.init.normal_(self.outConvLoc.weight, std=0.001)
        nn.init.constant_(self.outConvLoc.bias, 0)
