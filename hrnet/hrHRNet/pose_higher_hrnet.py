...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, num_points, **kwargs):
        self.inplanes = 64
        ...
        self.stage2, pre_stage_channels = self._make_stage(num_modules=1, num_branches=2, num_blocks=[4, 4],
                                                           num_channels=[24, 48], block=blocks_dict['BOTTLENECK'],
                                                           fuse_method='SUM', num_inchannels=num_channels)
            # self.stage2_cfg, num_channels)

        # self.stage3_cfg = cfg['MODEL']['EXTRA']['STAGE3']
        num_channels = [24, 48, 92]
        block = blocks_dict['BOTTLENECK']
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))
        ]
        ...
        self.stage3, pre_stage_channels = self._make_stage(num_modules=1, num_branches=3, num_blocks=[4, 4, 4],
                                                           num_channels=[24, 48, 92], block=blocks_dict['BOTTLENECK'],
                                                           fuse_method='SUM', num_inchannels=num_channels)

        # self.stage4_cfg = cfg['MODEL']['EXTRA']['STAGE4']
        num_channels = [24, 48, 92, 192]
        block = blocks_dict['BOTTLENECK']
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))
        ]
        self.transition3 = self._make_transition_layer(
            pre_stage_channels, num_channels)
        self.stage4, pre_stage_channels = self._make_stage(num_modules=1, num_branches=4, num_blocks=[4, 4, 4, 4],
                                                           num_channels=[24, 48, 92, 192],
                                                           block=blocks_dict['BOTTLENECK'], fuse_method='SUM',
                                                           num_inchannels=num_channels, multi_scale_output=False)
